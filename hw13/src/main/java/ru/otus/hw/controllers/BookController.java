...
public class BookController {
    ...
    public String allBooksPage(Model model) {
        List<BookDto> bookDtos = bookService.findAll().stream().map(bookMapper::bookToDto).toList();
    ...
    public String bookByIdPage(@RequestParam long bookId, Model model) throws Exception {
        BookDto book = bookMapper.bookToDto(bookService.findById(bookId).orElseThrow(() ->
    ...
    public String updateBook(Model model, @Valid @ModelAttribute("bookDto") UpdateBookDto bookDto,
        ...
        if (bindingResult.hasErrors()) {
            BookDto book = bookMapper.bookToDto(bookService.findById(bookDto.id()).orElseThrow(() ->
